...
import main as core
from api_auth import ApiContext, check_playground_rate_limit, get_api_context, get_session_user, require_auth_enabled
...
app = FastAPI(title="FireScrapling API", version="1.0.0")
...
def crawl_status(
    ...
    if ctx.user_id:
        ...
        rows = core.get_job_results_for_user(job_id, ctx.user_id)
        if rows is None:
            raise HTTPException(status_code=404, detail="Job not found")
        return {"id": job_id, "status": job.get("status"), "data": rows}
